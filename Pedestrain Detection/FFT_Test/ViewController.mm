...
- (void)viewDidLoad {
    ...
    results(count, 2) = vdsp_n_secs;
    count++;
    }

    
    //results.save("FFT_Comparision.txt", arma::raw_ascii);
    ...
    NSString *fileName = [NSString stringWithFormat:@"%@/FFT_Comparision.txt",
                          documentsDirectory];
    ...
    results.save(fname, arma::raw_ascii);
